...
export default function ShareProductButton({ name }: { name: string }) {
  ...
  return <button type="button" onClick={share} className="flex items-center gap-2 rounded-full border border-[#11110f]/15 px-5 py-3 text-[10px] font-extrabold uppercase tracking-[0.08em] hover:bg-[#11110f] hover:text-white"><Share2 className="h-4 w-4"/>{copied ? "Link copied" : "Share"}</button>;
}
